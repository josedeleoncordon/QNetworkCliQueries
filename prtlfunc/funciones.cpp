...
QString puertosDeAccesoATabla(QString pais, QList<MacBuscando*> lstARPs)
{
    QString salida;
    foreach (MacBuscando *mac, lstARPs)
    {
        QString equipo;
        QString equipoip;
        ...
        QString interfaz;
        QString descripcion;
        QString ultimoEsfuerzo;
        QString usandoHistorico;

        if ( !mac->interfaz.isEmpty() )
        {
            equipo = mac->equipo;
            equipoip = mac->equipoip;
            equipoplatform = mac->equipoplatform;
            interfaz = mac->interfaz;
            descripcion = mac->interfaceDescription;
            ultimoEsfuerzo = "No";
            if ( mac->usandoHistorico ) usandoHistorico="Si"; else usandoHistorico="No";
        }
        else if ( !mac->interfaceUltimoEsfuerzo.isEmpty() )
        {
            equipo = mac->equipoUltimoEsfuerzo;
            equipoip = mac->equipoipUltimoEsfuerzo;
            equipoplatform = mac->equipoplatformUltimoEsfuerzo;
            interfaz = mac->interfaceUltimoEsfuerzo;
            descripcion = mac->interfaceDescripcionUltimoEsfuerzo;
            ultimoEsfuerzo = "Si";
            if ( mac->usandoHistoricoUltimoEsfuerzo ) usandoHistorico="Si"; else usandoHistorico="No";
        }

        //No exportamos la informacion que no tiene equipo e interfaz
        if ( equipo.isEmpty() )
            continue;
        ...
        //segundo equipo. ATN ETX etc
        if ( !mac->equipo2.isEmpty() )
        {
            equipo = mac->equipo2;
            equipoip = mac->equipo2ip;
            equipoplatform = mac->equipo2platform;
            interfaz = mac->interfaz2;
            descripcion = mac->interface2Description;
            ultimoEsfuerzo = "No";
            ...
            salida.append(pais+"\t"+
                          ...
                          mac->PE+"\t"+
                          mac->vrf+"\t"+
                          ( mac->usandoHistoricoUltimoEsfuerzo?"Si":"No" )+"\t"+
                          ultimoEsfuerzo+"\t"+
                          equipoip+"\t"+
                          equipoplatform+"\t"+
                          equipo+"\t"+
...
QString puertosDeAccesoATablaRutaCompleta(QString pais,QList<MacBuscando*> lstARPs)
{
    QString salida;
    int maxPathSize=0;
    foreach (MacBuscando *mac, lstARPs)
        if ( mac->path.size() > maxPathSize )
            maxPathSize=mac->path.size();
    ...
    foreach (MacBuscando *mac, lstARPs)
    {
        ...
        if ( mac->path.size() > 1 )
        {
            QString lastInterface = mac->path.last();
            mac->path.removeLast();

            path = mac->path.join("\t");
            path.resize(path.size()+(maxPathSize-mac->path.size()),'\t');
            path.append(lastInterface);
        }
        else if ( path.size() == 1 )
            path = mac->path.last();
        else
        ...
        salida.append(pais+"\t"+
                      ...
                      mac->vlan+"\t"+
                      mac->mac+"\t"+
                      (mac->usandoHistorico?"Si":"No")+"\t"+
                      mac->interfaceDescription+"\t"+
                      path+
                      "\n");
    }
    return salida;
