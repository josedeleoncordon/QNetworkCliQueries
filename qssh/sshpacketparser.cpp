...
quint32 SshPacketParser::asUint32(const QByteArray &data, quint32 offset)
{
    ...
    quint32 uno = ((data.at(offset) & 0xff) << 24);
    quint32 dos = ((data.at(offset + 1) & 0xff) << 16);
    quint32 tres = ((data.at(offset + 2) & 0xff) << 8);
    quint32 cuatro = (data.at(offset + 3) & 0xff);

    const quint32 value = uno + dos + tres + cuatro;
    return value;
}
