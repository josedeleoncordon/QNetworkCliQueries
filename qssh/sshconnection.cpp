    ...
    : QObject(parent)
{
    ...
    connect(d, SIGNAL(reachable()),SIGNAL(reachable())); //JOSE
}
...
//    const QRegExp versionIdpattern(QString::fromLatin1("SSH-(%1)-%1(?: .+)?").arg(legalString));
    const QRegExp versionIdpattern("SSH-((\\.|\\d)+)-.+");
...
//        qDebug() << "mensaje" << msg.invalidMsgSeqNr << m_lastInvalidMsgSeqNr << QDateTime::currentDateTime();
//        throw SshServerException(SSH_DISCONNECT_PROTOCOL_ERROR,
...
{
    //***************************************** //Jose
    qDebug() << "SshConnectionPrivate::handleDisconnect()";

    QObject *o = sender();
    if ( o )
    {
        qDebug() << "este" << o << o->objectName();
