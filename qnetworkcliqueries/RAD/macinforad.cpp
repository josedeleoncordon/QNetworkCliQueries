...
{
    m_mac = m_queriesConfiguration.value("MAC_MAC",m_ip,m_os,m_conexionID,m_queryName);
    if ( m_platform == "ETX-1" )
    {
...
void MacInfoRAD::on_term_receiveTextETX1()
{
    ...
    foreach (QString line, lines)
    {
        ...
        if ( line.contains( expmac,&match ) )
        {
            ...
            QStringList vlanfilter = m_queriesConfiguration.values("MAC_vlansFilter",m_ip,m_os,m_conexionID,m_queryName);
